...
class TestArenaOriginAndCookieScoping(BaseBridgeTest):
    def test_detect_arena_origin(self) -> None:
        ...
        self.assertEqual(
            self.main._detect_arena_origin("https://lmarena.ai/?mode=direct"), "https://arena.ai"
        )
        self.assertEqual(
            self.main._detect_arena_origin("https://arena.ai/?mode=direct"), "https://arena.ai"
        )
        self.assertEqual(
            self.main._detect_arena_origin("https://www.arena.ai/foo"), "https://arena.ai"
        )
    ...
    def test_arena_origin_candidates(self) -> None:
        ...
        self.assertEqual(
            self.main._arena_origin_candidates(
    ...
    def test_arena_auth_cookie_specs_scope_to_both_origins(self) -> None:
        specs = self.main._arena_auth_cookie_specs(
            "base64-token-1", page_url="https://arena.ai/?mode=direct"
        )
    ...
    def test_provisional_user_id_cookie_specs_include_host_and_domain(self) -> None:
        specs = self.main._provisional_user_id_cookie_specs(
            "prov-1", page_url="https://arena.ai/?mode=direct"
        )
    ...
    async def test_get_arena_context_cookies_dedupes_by_name_domain_path(self) -> None:
        ...
        ctx = _FakeContext()
        cookies = await self.main._get_arena_context_cookies(
            ctx, page_url="https://arena.ai/?mode=direct"
        )
